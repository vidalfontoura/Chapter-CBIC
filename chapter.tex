...
Placing the residues in the 2D or 3D lattice can vary depending on the employed technique. There are two popular strategies used to place the residues in the space. The first is called Relative Positioning. The main idea is to place every residue but the first in relation to the previous one. The movements are defined as being \textit{left}, \textit{right} and \textit{forward} for a 2D space, and include \textit{front} and \textit{back} for a 3D space. The second technique is the Absolute Positioning, where each residue is placed giving its absolute position (x,y) or (x,y,z) in the space.
