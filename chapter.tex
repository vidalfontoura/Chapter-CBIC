...
However, many of these approaches make use only of some well-known algorithms like Genetic Algorithm, Particle Swarm Optimization and Ant Colony Optimization. Also there is a lack of works that explore multi-objective approaches. Gabriel \textit{et al}. \cite{gabriel2012algoritmos} presented an multi-objective approach that enables to distinguish between conformations with the same energy value but with different compactness. The multi-objective formulation presented by  Gabriel \textit{et al}. \cite{gabriel2012algoritmos} introduces a second objective which is concerned about the compactness of proteins structures. The multi-objective algorithm proposed by \cite{gabriel2012algoritmos} is not pareto dominance based \cite{pareto}, since it decomposes the objectives into mono-objective problems and optimizes them using sub-populations.

 This leave a window of opportunities speaking about classic pareto based algorithms like NSGAII \cite{zitzler1998multiobjective} and IBEA \cite{zitzler2004indicator} for example. The main difference between the mentioned methods \cite{custodio2004investigation, hsu2003growth, krasnogor2002multimeme, lin2011protein, unger1993genetic}, is that NSGAII and IBEA are multi-objective algorithms, and different from a GA, have more sophisticated mechanisms to evolve the population and try to improve the solutions. 
